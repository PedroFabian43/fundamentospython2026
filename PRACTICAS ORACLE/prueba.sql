...
SELECT COUNT(FUNCION) AS EMPLEADOS, FUNCION FROM PLANTILLA GROUP BY FUNCION ORDER BY FUNCION;

SELECT COUNT(*) AS EMPLEADOS, DEPT_NO, OFICIO FROM EMP GROUP BY DEPT_NO, OFICIO HAVING COUNT(*) >= 2;

SELECT AVG(SALARIO) AS MEDIASALARIO, COUNT(*) AS EMPLEADOS, FUNCION FROM PLANTILLA WHERE SALA_COD = 6 GROUP BY FUNCION;

SELECT * FROM EMP;  

SELECT MAX(FECHA_ALT) AS FECHA, OFICIO FROM EMP GROUP BY OFICIO ORDER BY MAX(FECHA_ALT);
